...
const Toolbar = (props) => {
    ...
    return (
        <div 
            className={["toolbar", isHovering && 'hover', isDroppable && 'droppable'].join(' ')}
            ref={dropRef}
            onClick={add}
            >
        ...
        <div className={['item', isHovering && 'hover'].join(' ')}>
            ...
            <div className="children">
                ...
                {itemData?.children?.map((child, childIndex) => {  
                    ...
                    return <TreeItem key={itemDataPosition.join('-')} data={data} dropData={dropData} dataPosition={itemDataPosition}
                        update={update} updateDrop={updateDrop} item={item} disabled={disabled} />
                })}
            </div>
        </div>
    )
...
const ItemContainer = (props) => {
    ...
    return (
        <div className={['container-row', isHovering && 'hover'].join(' ')}
        onMouseDown={onMouseDown}>
            ...
            {isShowDeleteBtn && <Popconfirm key="delete" title="确认要删除吗？" onConfirm={deleteItem} icon={<ExclamationCircleFilled style={{ color: 'red' }} />}>
        ...
        if(Array.isArray(itemData.children) && itemData.children.length>0){
            ...
            return <>
                ...
                {itemData.children.map((_, childIndex) => {
                  ...
                  return <>
                        {childIndex!==0 && <Tag color={color} style={{marginLeft: '8px'}}>{text}</Tag>}
                        <Child key={_._key}/>
                    </>
                })}
            <span style={bracketsStyle}>）</span>
...
const TreeItem = (props) => {
    ...
    const newGroup = ()=>{
        ...
        const _itemData = {
            ...
            children: [
                itemData,
                {...DEFAULT_ITEM_DATA, _key: generateKey()}
            ]
        }
        if(itemParent){
    ...
    return <div ref={disabled?null:dragRef} className={['drop-item', dragging && 'grab'].join(' ')}
        onMouseOver={onMouseEnter}
        onMouseLeave={onMouseLeave}
        >
        
        {!disabled && !isRoot && !isEmpty && ([0, undefined, null].includes(itemIndex)) && <Toolbar add={addBefore}
            title={'往前加'} drop={dropBefore} isDroppable={isDroppable} />}
        <div className={['drop-main', isHovering && 'hover', !disabled && isDraggable && 'grab'].join(' ')} >
            
            <div className='expand-icon'>
                {ItemWrapper === BranchItem && <>
                    {isExpand ? <MinusSquareOutlined onMouseDown={closeExpand} />:<PlusSquareOutlined onMouseDown={openExpand}/>}
                </>}
            </div>

            {isExpand && <>
                <ItemWrapper data={data} dropData={dropData} dataPosition={dataPosition} update={update} updateDrop={updateDrop} newGroup={newGroup} deleteItem={deleteItem} add={addBefore} drop={dropBefore} isDroppable={isDroppable} item={item} disabled={disabled} />
            </>}

            {!isExpand && <Tooltip><TooltipText/></Tooltip>}
        </div>
